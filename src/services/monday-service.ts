...
export async function checkAndAssignUser(boardId: number, itemId: number, userId: number, token: string | undefined) {
  try {
    const mondayClient = initMondayClient()
    await mondayClient.setToken(token)
    ...
    const queryValue = JSON.parse(queryRes.data.boards[0].items[0].column_values[0].value)
...
export async function getDesignatedBoardId(userTeamName: string | undefined, token: string | undefined) {
  const mondayClient = initMondayClient()
  await mondayClient.setToken(token)
  ...
  const designatedBoardId = queryRes.data.boards.find((board) => board.name === userTeamName).id
